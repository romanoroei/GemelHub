...
const App = (() => {
  ...
  // ── Professional PDF export (js/pdf-export.js) ─────────────────────────
  // Primary export path for the two fund reports (portfolio summary +
  // compare). window.print() remains wired as _sbPrintSummary/_sbPrintCompare
  // above and is used as an automatic fallback if PDF generation fails for
  // any reason (library failed to load, font fetch failed, etc.) — see the
  // catch blocks in _sbExportSummaryPdf/_sbExportComparePdf below.
  const SB_PDF_DISCLAIMER = 'המידע נועד לספק תמונת מצב כללית והשוואתית בלבד ואינו מהווה ייעוץ השקעות, שיווק פנסיוני או תחליף לייעוץ אישי המותאם לצרכי הלקוח. הנתונים מבוססים על מקורות פומביים ועשויים להכיל טעויות או אי-דיוקים. אין לראות בתשואות העבר התחייבות לתשואות עתידיות. לפני קבלת החלטה פיננסית מומלץ להתייעץ עם בעל רישיון.';
  const SB_PDF_AGENT_LINE = 'רועי רומנו, מתכנן פיננסי וסוכן פנסיוני מורשה | 052-8089808';
  const SB_PDF_LOGO_URL = 'assets/gemelhub-logo-light.svg';

  function _pdfYieldColor(val) {
    const n = parseFloat(val);
    if (isNaN(n) || val === '' || val == null) return '#94a3b8';
    return n >= 0 ? '#16a34a' : '#dc2626';
  }

  function _pdfFundUrl(fundId, categoryId) {
    return new URL(`fund.html?id=${encodeURIComponent(fundId || '')}&cat=${encodeURIComponent(categoryId || '')}`, location.href).toString();
  }

  // Builds the data model for the single-portfolio "המעבדה שלי" report —
  // mirrors the grouping/columns already computed in renderSandboxPage()
  // and _sbBuildPrintValueSummaryHtml(), but as plain data instead of HTML.
  function _sbBuildSummaryPdfModel() {
    const portfolio = (state.sandbox.portfolio || []).filter(it => !it.hidden);
    const now = new Date();
    const dateStr = now.toLocaleDateString('he-IL', { day: '2-digit', month: '2-digit', year: 'numeric' });
    const title = state.sandbox.portfolioName || _sbDefaultPortfolioName();

    const grouped = {};
    portfolio.forEach(item => { (grouped[item.categoryId] = grouped[item.categoryId] || []).push(item); });
    const catOrder = CONFIG.PRODUCT_CATEGORIES.map(c => c.id);
    const sortedKeys = Object.keys(grouped).sort((a, b) => catOrder.indexOf(a) - catOrder.indexOf(b));

    const blocks = [];

    // value summary card (mirrors _sbBuildPrintValueSummaryHtml)
    const cardRows = sortedKeys.map(catId => {
      const items = grouped[catId];
      const meta = _sbGetCategoryMeta(catId);
      const amountTotal = items.filter(it => it.investMode === 'amount' && it.investAmount !== '')
        .reduce((s, it) => s + (parseFloat(String(it.investAmount).replace(/,/g, '')) || 0), 0);
      const pctTotal = items.filter(it => it.investMode === 'percent' && it.investPct !== '')
        .reduce((s, it) => s + (parseFloat(it.investPct) || 0), 0);
      const amountText = amountTotal > 0 ? `${Math.round(amountTotal).toLocaleString('he-IL')} ש"ח` : '';
      const pctText = pctTotal > 0 ? `${pctTotal.toFixed(pctTotal % 1 ? 1 : 0)}%` : '';
      const valueText = amountText && pctText ? `${amountText} · ${pctText}` : (amountText || pctText || 'ללא סכום');
      const managerCount = new Set(items.map(it => String(it.provider || '').trim()).filter(Boolean)).size;
      return { label: meta.label, value: valueText, sub: managerCount ? (managerCount === 1 ? 'מנהל השקעות 1' : `${managerCount} מנהלי השקעות`) : '' };
    });
    const allInvestedAsAmount = portfolio.length > 0 && portfolio.every(item => item.investMode === 'amount');
    const totalAmount = allInvestedAsAmount
      ? portfolio.reduce((sum, item) => sum + (parseFloat(String(item.investAmount).replace(/,/g, '')) || 0), 0)
    ...
    if (cardRows.length) {
      blocks.push({
        type: 'valueCards',
        cards: [{
          name: '', totalLabel: 'שווי התיק שלי',
          total: totalAmount > 0 ? `${Math.round(totalAmount).toLocaleString('he-IL')} ש"ח` : '',
          rows: cardRows.map(r => ({ label: r.label, value: r.value, sub: r.sub })),
        }],
      });
    }
    ...
    sortedKeys.forEach(catId => {
      ...
      const columns = [
        { header: 'מנהל', weight: 1.3 },
        { header: 'מסלול', weight: 1.7 },
        { header: 'דמי ניהול מצבירה %', weight: 1, align: 'center' },
      ];
      if (isPension) columns.push({ header: 'דמי ניהול מהפקדה %', weight: 1, align: 'center' });
      columns.push({ header: 'השקעה', weight: 0.9, align: 'center' });
      returnFields.forEach(f => columns.push({ header: _sbReturnFieldLabel(f, monthlyLabel), weight: 0.65, align: 'center' }));
      columns.push({ header: '% מניות', weight: 0.65, align: 'center' });
      columns.push({ header: '% חו"ל', weight: 0.65, align: 'center' });
      columns.push({ header: '% מט"ח', weight: 0.65, align: 'center' });

      const rows = items.map(item => {
        const rawInvestVal = item.investMode === 'percent' ? item.investPct : item.investAmount;
        const displayInvest = rawInvestVal !== '' && rawInvestVal != null
          ? Number(String(rawInvestVal).replace(/,/g, '')).toLocaleString('he-IL') + (item.investMode === 'percent' ? '%' : '')
          : '—';
        const cells = [
          { text: item.provider || '', bold: true, color: providerColor(String(item.provider || '').trim()), link: _pdfFundUrl(item.fundId, item.categoryId) },
          { text: item.trackLabel || '', sub: item.fundId ? `#${item.fundId}` : '' },
          { text: item.dnCumulative !== '' ? `${item.dnCumulative}%` : '—', align: 'center' },
        ];
        if (isPension) cells.push({ text: item.dnDeposit !== '' ? `${item.dnDeposit}%` : '—', align: 'center' });
        cells.push({ text: displayInvest, align: 'center' });
        returnFields.forEach(f => {
          const v = item[f.itemKey];
          cells.push({ text: _sbFmtPct(v), align: 'center', color: _pdfYieldColor(v) });
        });
        cells.push({ text: item.stock !== '' ? _sbFmtPct(item.stock, 0) : '—', align: 'center' });
        cells.push({ text: item.abroad !== '' ? _sbFmtPct(item.abroad, 0) : '—', align: 'center' });
        cells.push({ text: item.fx !== '' ? _sbFmtPct(item.fx, 0) : '—', align: 'center' });
        return cells;
      });

      blocks.push({ type: 'sectionTitle', text: `${meta.label} (${items.length})`, color: meta.color });
      blocks.push({ type: 'table', columns, rows });
    });

    return {
      title, dateStr, meta: SB_PDF_AGENT_LINE, logoUrl: SB_PDF_LOGO_URL,
      blocks, disclaimer: SB_PDF_DISCLAIMER,
    };
  }

  // Builds the data model for the "השוואת תיקים" report — mirrors
  // _sbRenderCompare()'s sections (value cards, tracks per category, returns,
  // exposures) as plain data. Note: unlike the summary report, the compare
  // view never linked provider names to fund.html (plain text there too),
  // so there are no link annotations to preserve here.
  function _sbBuildComparePdfModel(rawItems) {
    const items = rawItems.map(it => ({ ...it, portfolio: (it.portfolio || []).filter(t => !t.hidden) }));
    const n = items.length;
    const sums = items.map(it => _sbBuildExtendedSummary(it.portfolio));
    const now = new Date();
    const dateStr = now.toLocaleDateString('he-IL', { day: '2-digit', month: '2-digit', year: 'numeric' });
    const title = n === 2 ? `השוואה: ${items[0].name} vs ${items[1].name}` : `השוואת ${n} תיקים`;
    const catOrder = CONFIG.PRODUCT_CATEGORIES.map(c => c.id);
    ...
    const cards = items.map(it => {
      ...
      const totalAmount = allInvestedAsAmount
        ? portfolio.reduce((sum, item) => sum + (parseFloat(String(item.investAmount || '').replace(/,/g, '')) || 0), 0)
        : 0;
      const rows = Object.keys(grouped).sort((a, b) => catOrder.indexOf(a) - catOrder.indexOf(b)).map(catId => {
        const tracks = grouped[catId];
        const meta = _sbGetCategoryMeta(catId);
        const label = tracks[0]?.categoryLabel || meta.label || catId;
        const amountTotal = tracks.filter(t => t.investMode === 'amount' && t.investAmount !== '')
          .reduce((sum, t) => sum + (parseFloat(String(t.investAmount).replace(/,/g, '')) || 0), 0);
        const pctTotal = tracks.filter(t => t.investMode === 'percent' && t.investPct !== '')
          .reduce((sum, t) => sum + (parseFloat(String(t.investPct).replace(/,/g, '')) || 0), 0);
        const amountText = amountTotal > 0 ? `₪${Math.round(amountTotal).toLocaleString('he-IL')}` : '';
        const pctText = pctTotal > 0 ? `${pctTotal.toFixed(pctTotal % 1 ? 1 : 0)}%` : '';
        const valueText = amountText && pctText ? `${amountText} · ${pctText}` : (amountText || pctText || 'ללא סכום');
        const managerCount = new Set(tracks.map(t => String(t.provider || '').trim()).filter(Boolean)).size;
        return { label, value: valueText, sub: managerCount ? (managerCount === 1 ? 'מנהל השקעות 1' : `${managerCount} מנהלי השקעות`) : '' };
      });
      return {
        name: it.name, totalLabel: 'שווי התיק',
        total: totalAmount > 0 ? `₪${Math.round(totalAmount).toLocaleString('he-IL')}` : '—',
        rows,
      };
    });
    blocks.push({ type: 'sectionTitle', text: 'שווי ופירוט התיקים' });
    blocks.push({ type: 'valueCards', cards });

    // ── tracks per category
    const allCats = {};
    items.forEach(it => it.portfolio.forEach(t => { if (!allCats[t.categoryId]) allCats[t.categoryId] = t.categoryLabel || t.categoryId; }));
    const catIds = Object.keys(allCats);
    if (catIds.length) {
      blocks.push({ type: 'sectionTitle', text: 'מסלולים לפי קטגוריה' });
      const trackColumns = [{ header: 'קטגוריה', weight: 0.8 }, ...items.map(it => ({ header: it.name, weight: 1.3 }))];
      const trackRows = catIds.map(catId => {
        const cells = [{ text: allCats[catId], bold: true }];
        items.forEach(it => {
          const tracks = it.portfolio.filter(t => t.categoryId === catId);
          if (!tracks.length) { cells.push({ text: '—' }); return; }
          const catTot = tracks.reduce((s, t) => s + (parseFloat(String(t.investAmount || '').replace(/,/g, '')) || 0), 0);
          const desc = tracks.map(t => {
            const amt = parseFloat(String(t.investAmount || '').replace(/,/g, '')) || 0;
            const amtStr = amt > 0 ? ` (₪${Math.round(amt).toLocaleString('he-IL')})` : '';
            return `${t.provider || ''}${t.trackLabel ? ' — ' + t.trackLabel : ''}${amtStr}`;
          }).join('  |  ');
          let wFee = 0, wSum = 0;
          tracks.forEach(t => {
            const w = parseFloat(String(t.investAmount || '').replace(/,/g, '')) || 0;
          ...
          const avgFee = wSum > 0 ? (wFee / wSum).toFixed(2) + '%' : '—';
          cells.push({ text: `${desc}   ·   סה"כ: ${catTot > 0 ? '₪' + Math.round(catTot).toLocaleString('he-IL') : '—'}, ד"נ ממוצע: ${avgFee}` });
        });
        return cells;
      });
      blocks.push({ type: 'table', columns: trackColumns, rows: trackRows });
    }

    // ── returns & fees / exposures tables (shared builder)
    const mkCompareTable = (headLabel, rows) => {
      const columns = [{ header: headLabel, weight: 1 }, ...items.map(it => ({ header: it.name, weight: 0.9, align: 'center' }))];
      if (n === 2) columns.push({ header: 'הפרש', weight: 0.9, align: 'center' });
      const tableRows = rows.map(row => {
        const vals = sums.map(s => s[row.key]);
        const best = row.isReturn ? Math.max(...vals.filter(v => v != null)) : null;
        const cells = [{ text: row.label, bold: true }];
        vals.forEach(v => {
          if (v == null) { cells.push({ text: '—', align: 'center' }); return; }
          const color = row.isReturn ? _pdfYieldColor(v) : undefined;
          const star = row.isReturn && v === best ? '★ ' : '';
          cells.push({ text: `${star}${v.toFixed(row.dec || 2)}%`, align: 'center', color });
        });
        if (n === 2 && vals[0] != null && vals[1] != null) {
          const d = vals[0] - vals[1];
          cells.push({ text: `${d >= 0 ? '+' : ''}${d.toFixed(row.dec || 2)}%`, align: 'center', color: row.isReturn ? _pdfYieldColor(d) : undefined });
        } else if (n === 2) { cells.push({ text: '—', align: 'center' }); }
        return cells;
      });
    ...
    const hasDeposit = items.some(it => it.portfolio.some(t => parseFloat(t.dnDeposit) > 0));
    const feeRows = [{ label: 'ד"נ מצבירה', key: 'avgFee', dec: 2 }];
    if (hasDeposit) feeRows.push({ label: 'ד"נ מהפקדה', key: 'avgDnDeposit', dec: 2 });
    const returnsTable = mkCompareTable('תקופה', [
      { label: 'חודש אחרון', key: 'avgY1', dec: 2, isReturn: true },
      { label: '3 חודשים', key: 'avgY3m', dec: 2, isReturn: true },
      { label: '12 חודשים', key: 'avgY12', dec: 2, isReturn: true },
      { label: '3 שנים', key: 'avgY3y', dec: 2, isReturn: true },
    ...
    blocks.push({ type: 'sectionTitle', text: 'תשואות ודמי ניהול (ממוצע משוקלל)' });
    blocks.push({ type: 'table', columns: returnsTable.columns, rows: returnsTable.rows });

    const exposuresTable = mkCompareTable('חשיפה', [
      { label: 'מניות', key: 'avgStock', dec: 1 },
      { label: 'חו"ל', key: 'avgAbroad', dec: 1 },
      { label: 'מט"ח', key: 'avgFx', dec: 1 },
    ]);
    blocks.push({ type: 'sectionTitle', text: 'חשיפות (ממוצע משוקלל)' });
    blocks.push({ type: 'table', columns: exposuresTable.columns, rows: exposuresTable.rows });

    return { title, dateStr, meta: SB_PDF_AGENT_LINE, logoUrl: SB_PDF_LOGO_URL, blocks, disclaimer: SB_PDF_DISCLAIMER };
  }

  async function _sbExportSummaryPdf() {
    if (!(state.sandbox.portfolio || []).some(it => !it.hidden)) { showToast('התיק ריק'); return; }
    try {
      if (!window.GemelHubPdfExport) throw new Error('pdf export engine not loaded');
      showToast('מכינים PDF...');
      const model = _sbBuildSummaryPdfModel();
      const bytes = await window.GemelHubPdfExport.buildReportPdf(model);
      const result = await window.GemelHubPdfExport.deliverPdf(bytes, `${model.title || 'תיק השקעות'}.pdf`);
      if (result === 'downloaded') showToast('ה-PDF ירד בהצלחה');
    } catch (err) {
      console.warn('GemelHub: PDF export failed, falling back to window.print()', err);
      showToast('לא הצלחנו ליצור PDF — פותחים הדפסה רגילה במקום');
      _sbPrintSummary();
    }
  }

  async function _sbExportComparePdf() {
    const items = state.sandbox.compareItems || [];
    if (items.length < 2) { showToast('אין מספיק תיקים להשוואה'); return; }
    try {
      if (!window.GemelHubPdfExport) throw new Error('pdf export engine not loaded');
      showToast('מכינים PDF...');
      const model = _sbBuildComparePdfModel(items);
      const bytes = await window.GemelHubPdfExport.buildReportPdf(model);
      const result = await window.GemelHubPdfExport.deliverPdf(bytes, `${model.title || 'השוואת תיקים'}.pdf`);
      if (result === 'downloaded') showToast('ה-PDF ירד בהצלחה');
    } catch (err) {
      console.warn('GemelHub: PDF export failed, falling back to window.print()', err);
      showToast('לא הצלחנו ליצור PDF — פותחים הדפסה רגילה במקום');
  ...
  function setupSandboxPortfolioDialogs() {
    ...
    document.getElementById('sb-compare-print-btn')?.addEventListener('click', _sbExportComparePdf);
  ...
  function _sbAttachEvents(section) {
    ...
    section.querySelector('#sandbox-print-btn')?.addEventListener('click', _sbExportSummaryPdf);
